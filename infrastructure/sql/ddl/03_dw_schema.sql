...
CREATE TABLE IF NOT EXISTS dw.dim_member (
    ...
    -- SHA256 of business attrs for change detection
    attr_hash CHAR(64) NOT NULL,
    load_id BIGINT,                      -- staging.load_batches reference
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (member_id, validity_start_ts)
);
CREATE INDEX IF NOT EXISTS idx_dim_member_member_id_current ON dw.dim_member (
    member_id
) WHERE current_flag;
CREATE INDEX IF NOT EXISTS idx_dim_member_attr_hash_current ON dw.dim_member (
    attr_hash
) WHERE current_flag;
...
CREATE INDEX IF NOT EXISTS idx_dim_provider_provider_id_current ON dw.dim_provider (
    provider_id
) WHERE current_flag;
...
CREATE INDEX IF NOT EXISTS idx_dim_plan_nk_current ON dw.dim_plan (
    plan_id, effective_year
) WHERE current_flag;
...
CREATE INDEX IF NOT EXISTS idx_fact_claim_member_sk ON dw.fact_claim (
    member_sk
);
CREATE INDEX IF NOT EXISTS idx_fact_claim_provider_sk ON dw.fact_claim (
    provider_sk
);
...
CREATE INDEX IF NOT EXISTS idx_fact_enrollment_member_sk ON dw.fact_enrollment (
    member_sk
);
CREATE INDEX IF NOT EXISTS idx_fact_enrollment_plan_sk ON dw.fact_enrollment (
    plan_sk
);
CREATE INDEX IF NOT EXISTS idx_fact_enrollment_start_date_key ON dw.fact_enrollment (
    start_date_key
);
CREATE INDEX IF NOT EXISTS idx_fact_enrollment_end_date_key ON dw.fact_enrollment (
    end_date_key
);
